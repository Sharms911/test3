...
def results():
    ...
    wind_speed_imperial = wind_speed/1.609

    dt = datetime.fromtimestamp(timestamp)
    temp_max = response.get("temp_max")
    temp_day = response.get("temp_day")
    temp_night = response.get("temp_eve")
    temp_eve = response.get("temp_eve")
    temp_morn = response.get("temp_morn")
    feels_like = response.get("feels_like")

    temp_max_c = round(temp_max - 273.15)
    temp_day_c = round(temp_day - 273.15)
    temp_night_c = round(temp_night - 273.15)
    temp_eve_c = round(temp_eve - 273.15)
    temp_morn_c = round(temp_morn - 273.15)
